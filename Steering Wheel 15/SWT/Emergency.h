...
void check_for_emergency()	{
	...
	if (eFlag && !eFlag1){
		...
		line_color 	= RGB565(BLACK_COW); 		//Line Color	
	...
	if (!eFlag && !eFlag2){
		back_color	= RGB565(INDIGO); 			//Background Color
		front_color = RGB565(MILK_WHITE); 		//Foreground Color
		line_color 	= RGB565(METALLIC_SILVER); 	//Line Color	
		text_color 	= RGB565(EMERALD_GREEN); 	//Text color
		resetLCD(emergencyMessage);
